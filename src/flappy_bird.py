...
# [TODO] Find a way to not show the video so that I can do the training in a Paperspace job
...
class FlappyBird(object):
    ...
    def next_frame(self, action):
        ...
        for pipe in self.pipes:
            self.screen.blit(
                self.pipe_images[0], (pipe["x_upper"], pipe["y_upper"]))
            self.screen.blit(
                self.pipe_images[1], (pipe["x_lower"], pipe["y_lower"]))
